    ...
    while (pContext->_bContinue) {
        ...
        if(ret != 0 || pSampleNode == NULL) {
            ...
            if (freqDetected>0)
            {
                ...
                extern SwiftCallbackFunc swiftFunc;
                
                swiftFunc(freqDetected);
            }
            // delete sample node.
            delete pSampleNode;
        }
...
void MusicAnalyzer::musicListener (pitch_idx_t detectedIdx, int pitchCounter, Pitch & pitchNode)
{
	if (pitchCounter > 5)
	{
        ...
        swiftPitch._pitchIndex = pitchNode._pitchIndex;
        swiftPitch._frequency = pitchNode._frequency;
        swiftPitch._totalPitchCounter = pitchNode._totalPitchCounter;
        swiftPitch._maxPositiveDeviation = pitchNode._maxPositiveDeviation;
        swiftPitch._maxNegativeDeviation = pitchNode._maxNegativeDeviation;
        swiftPitch._avgPositiveDeviation = pitchNode._avgPositiveDeviation;
        swiftPitch._avgNegativeDeviation = pitchNode._avgNegativeDeviation;
        swiftPitch._duration = pitchNode._duration;
	}
}
